# -*- coding: utf-8 -*-
"""Render out/sweep/data.json as docs/sweep-report.html (English) and
docs/sweep-report_ja.html (Japanese).
...
TEXT['en'] = dict(
    ...
    title='%(n)s scenarios you might (or might not) follow',
    lede=(
        'For an average, invented household, every combination of three conditions you decide and two you cannot '
        'was turned into a scenario, %(n)s in all. Under some conditions the household goes bankrupt. '
    ...
    h_path='How this simulator computes',
    sub_path='Net worth by year, real terms, hundred-million yen',
    ylabel_path='Net worth (¥100M, %(y0)s prices)', xlabel_path='Year',
    th_econ_oku='Economy (¥100M)', th_housing_oku='Housing (¥100M)',
    p_path=(
        'Pick one option from each of several conditions, and the simulator computes the path of the assets year by year, '
        'adding up the pay, the pension, tax, social insurance, the living cost, the rent or the mortgage, and the return on what is held. '
        'That shows the future those choices lead to. '
        'Running it for every combination of every option of every condition shows how much each condition matters. '
        'The lines in the next figure are the balance of the assets deflated to %(y0)s prices. '
        'The year a line drops below zero is the year the assets run out; we call it the ruin year.'),
    cap_path_house=(
        '<b>Buying against renting: the asset paths compared in the %(baseEcon)s economy the plan assumes.</b> '
        'Housing is the largest condition the household itself chooses. '
        'Both start from the same %(y0)s balance, and the owner keeps the flat after the mortgage is paid off. '
        'The renting plan moves to a one-bedroom in %(moveYear)s, once the children have left. '
        'The final assets of the two plans differ by %(gapLast)s hundred-million yen. '
        'The table below holds housing at each option and aggregates every combination of the rest.'),
    cap_path_econ=(
    ...
    sub_eta='Effect size of each condition (η², share of variance explained)',
    p_eta=(
        'Roughly, the effect size is how much a condition matters. '
        'Precisely, it is the share of the variance of the final assets that the differences between the means of that '
        "condition's options account for. A condition whose options barely move the mean has a small effect size."),
    th_share='Effect size', th_eta='Effect size (η²)',
    cap_eta=(
        'Orange bars are conditions the household cannot choose; blue bars are decisions it can make. '
        'The effect sizes of the five conditions acting separately add up to <b>%(etaMain)s</b> of the variance. The remainder is '
        'interaction: conditions acting together, where the effect of one depends on the option chosen for another. '
        'The simulator is deterministic and draws no random numbers, so all of the remainder is interaction.'),
    ...
    p_dials_note=(
        ...
        '%(smallVerb)s for under 1%% of the variance. That is a result too. It says which questions the household can '
    ...
    foot_gen=(
        ...
        '<code>go run ./tools/sweep &amp;&amp; python3 docs/analyse.py &amp;&amp; python3 docs/build.py</code>.'),
    foot_caveat=(
...
TEXT['ja'] = dict(
    ...
    title='あなたがたどりうる（かもしれない）%(n)s通りのシナリオ',
    lede=(
        '平均的な架空の世帯について、自分で決められる3つの条件と決められない2つの条件のすべての組み合わせで%(n)s通りのシナリオを作成した。'
        '条件によっては破産シナリオもある。'
        '全通りの組み合わせを計算する目的は<strong>どの条件がどの程度資産の推移を左右するか</strong>を見ることにある。'),

    h_path='このシミュレータの計算方法',
    sub_path='年ごとの純資産、実質値、億円',
    ylabel_path='純資産（億円、%(y0)s年物価）', xlabel_path='西暦',
    th_econ_oku='経済（億円）', th_housing_oku='住まい（億円）',
    p_path=(
        '複数の条件からそれぞれ選択肢を1つずつ選ぶと、シミュレータは年ごとに給与、年金、税、社会保険料、生活費、家賃または住宅ローン、'
        '保有資産の運用益を足し引きして資産推移を計算する。これによってその選択をした場合の未来を想像できる。'
        'さらにすべての条件のすべての選択肢の組み合わせで実行するとそれぞれの条件の効果量がわかる。'
        '次図の線は資産の残高を%(y0)s年の物価に割り引いたものである。線が0を割った年は資産が尽きた年でこれを破産年とよぶ。'),
    cap_path_house=(
        '<b>プランが想定する%(baseEcon)sの経済での持ち家と賃貸の資産推移の比較。</b>世帯自身が選べる条件では住まいが最も大きい。'
        'どちらも%(y0)s年の同じ残高から始まり、持ち家はローンを返し終えたあとも住まいが残る。'
        '賃貸は子が自立した%(moveYear)s年に1LDKへ移るプランとしている。この2つのプランの最終資産額には%(gapLast)s億円の差がつく。'
        '下の表は、持ち家と賃貸をそれぞれ固定し、そのほかの全組み合わせで集計したもの。'),
    cap_path_econ=(
    ...
    h_eta='どの条件の効果量が大きいか',
    sub_eta='各条件の効果量（η²、説明された分散の割合）',
    p_eta=(
        '効果量は大雑把にはその条件の重大さである。'
        '厳密には、最終資産の分散のうち、その条件の選択肢ごとの平均値の違いで説明できる割合である。'
        '選択肢を変えても平均値がほとんど動かない条件は、効果量が小さい。'),
    th_share='効果量', th_eta='効果量（η²）',
    cap_eta=(
        'オレンジは世帯が選べない条件、青は世帯が決められる条件。'
        '5つの条件それぞれ単独の効果量を合計すると分散の<b>%(etaMain)s</b>を説明する。'
        '残りは交互作用、つまり条件どうしが組み合わさって生じる効果で、ある条件の効き方が別の条件の選択肢によって変わる分である。'
        'シミュレータは決定的で乱数を含まないので、残りのすべてが交互作用である。'),
    ...
    p_dials_note=(
        '表の行どうしがほとんど変わらない意思決定は、平均的な世帯ではほとんど結果を左右しない。'
        '%(smallDials)sの効果量は1%%未満である。'
    ...
    h_diy='自分の条件で試算する方法',
    p_diy=(
        'シミュレータも全組み合わせの計算もリポジトリに含まれており平均的な世帯に固有の部分はない。'
        '差し替えるのは次の3つである。'),
    diy_steps=[
    ...
    foot_gen=(
        ...
        '<code>go run ./tools/sweep &amp;&amp; python3 docs/analyse.py &amp;&amp; python3 docs/build.py</code>を実行する。'),
    foot_caveat=(
...
SITE = 'https://kuniwak.github.io/ai-lifeplan/'
OG_IMAGE = 'https://ogimage.blog.st-hatena.com/11696248318752538694/14945776032049095917/1788752824'

SKELETON = '''<!doctype html>
<!-- generated by tools/sweep + docs/build.py; do not edit by hand -->
<html lang="{lang}">
<meta charset="utf-8">
...
<meta property="og:site_name" content="lifeplan">
...
<p>{p_eta}</p>
<figure><div class="scroll"><table>
<thead><tr><th>{th_axis}</th><th class="b">{th_share}</th><th class="n">{th_eta}</th></tr></thead>
...
def chart(lines, marks=(), title='', ylabel='', xlabel=''):
    ...
    for y in range(years[0] + (10 - years[0] % 10) % 10, years[-1] + 1, 10):
        o.append('<text class="ax" x="%.1f" y="%d" text-anchor="middle">%d</text>' % (X(y), Hc - B + 20, y))
    o.append('<text class="ax" x="%.1f" y="%d" text-anchor="middle">%s</text>' % ((L + Wc - R) / 2, Hc - 6, xlabel))
    o.append('<text class="ax" transform="translate(14,%.1f) rotate(-90)" text-anchor="middle">%s</text>' % ((T + Hc - B) / 2, ylabel))
...
def render(lang, page):
    ...
    nums = dict(
        ...
        ownRuinN=(len(own_ruin_econs) if lang == 'en' else '%dつ' % len(own_ruin_econs)), ownRuinEcons=join(own_ruin_econs),
    )
    out = {}
    for k, v in T.items():
        if k == 'axes':
    ...
    out['chart_house'] = chart([(OWN, own_base['values'], 'l1', False), (RENT, rent_base['values'], 'l2', False)],
                               marks=[(MOVE_YEAR, move_label)], title=T['sub_path'], ylabel=out['ylabel_path'], xlabel=out['xlabel_path'])
    out['chart_econ'] = chart([(l['econ'], l['values'], 'l1' if l['econ'] == BASE_ECON else 'ld', l['econ'] != BASE_ECON)
                               for l in SER['lines'] if l['house'] == OWN], title=T['sub_path'], ylabel=out['ylabel_path'], xlabel=out['xlabel_path'])
    out['css'] = CSS
    out['og_url'] = SITE + page
    out['og_image'] = OG_IMAGE
    out['og_description'] = strip_tags(out['lede'])
    return SKELETON.format(**out)


if __name__ == '__main__':
    for lang, path in (('en', 'docs/sweep-report.html'), ('ja', 'docs/sweep-report_ja.html')):
        html = render(lang, path[len('docs/'):])
